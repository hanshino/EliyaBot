$(document).ready(function () {
  ...
  socket.on("url added", function (url) {
    var shareUrl = `${location.origin}/${url.id}`;
    ...
    if (!charLoaded) {
      ...
      data.forEach(function (unit) {
        ...
        info
          .find(".Attribute")
          .removeClass()
          .addClass("Attribute " + unit.Attribute)
          .html("<span></span>");
        info
          .find(".Rarity")
          .removeClass()
          .addClass("Rarity Rarity" + unit.Rarity)
          .html("<span></span>");
        info
          .find(".Role")
          .removeClass()
          .addClass("Role " + unit.Role);
        elem.append(info);
          ...
          info
            ...
            .html(
              '<a href="' +
                ...
                "chars/" +
                unit.DevNicknames +
                '/full_shot_0.png" target="_blank"><img src="' +
                assetPath +
                "chars/" +
                unit.DevNicknames +
                '/full_shot_0.png" class="mainArt"></a><a href="' +
                assetPath +
                "chars/" +
                unit.DevNicknames +
                '/full_shot_1.png" target="_blank"><img src="' +
                assetPath +
                "chars/" +
                unit.DevNicknames +
                '/full_shot_1.png" class="altArt"></a>'
            );
          info
            .find(".Attribute")
            .removeClass()
  ...
  socket.on("equips", function (data) {
    if (!equipLoaded) {
      ...
      data.forEach(function (unit) {
        ...
        var attr = "";
        if (unit.Attribute == "All") {
          attr = getTls("AttributeAll");
        }
        info
          .find(".Attribute")
          .removeClass()
  ...
  $("#btnSave").on("click", function () {
    ...
    window.history.pushState("saved", "", location.origin);
    $(this).removeClass("on");
    setTimeout(function () {
      $("#btnSave").addClass("on");
    }, 100);
  ...
  $("#btnGetCompURL").on("click", function () {
    ...
    const imageUrl = `${location.origin}/comp/${units.join("-") + lngcode}.png`;
